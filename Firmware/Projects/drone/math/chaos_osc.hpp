...
public:
    ...
    ChaosOsc(M model, StateType initial_state, float sampling_frequency, float freq_multiplier, float max_dt = math::DT_DEFAULT)
        : sampling_frequency(sampling_frequency),
        freq_multiplier(freq_multiplier), max_dt(max_dt),
        model(model, 0.0f), state(initial_state)
    {
        recalculate_params();
    }
    ...
    void set_model(M new_model) {
        model.model = new_model;
    }

    float get_dt_overshoot() const {
        return dt_overshoot;
    }
